<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">站点报表</span>
			</div>
	...
	export default {
		...
		methods: {
			...
			first() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/report/EBF5CC3387DA41F198A05C03C2D3ABEB?token=" + toKen + "&page=" + this.pagenum +
						"&row=10")
					.then(res => {
						if (res.status == 200) {
			...
			searchData() {
				if (this.isBg == 1) {
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/17CA41FB8FAC49C8B973BE9AC3CA366D?token=" + toKen + "&page=" + this.pagenum +
							"&row=10&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/54BE9E41573D4EC9BE11709C6A5ECDA1?token=" + toKen + "&page=" + this.pagenum +
							"&row=10&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
