<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">运营报表</span>
			</div>
	...
	export default {
		...
		methods: {
			...
			first() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/report/EE252E4650B842798077F99DE5475039?token=" + toKen + "&page=" + this.pagenum +
						"&row=10")
					.then(res => {
						if (res.status == 200) {
			...
			searchData() {
				if (this.isBg == 1) {
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/AB5E1BDE13F145919918CDB802505128?token=" + toKen + "&page=" + this.pagenum +
							"&row=10&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/26ECF579E3D84203A9DB0F8D726AF892?token=" + toKen + "&page=" + this.pagenum +
							"&row=10&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
