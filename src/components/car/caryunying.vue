<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">运营商报表</span>
			</div>
			...
			<div style="height: 680px;">
				<table class="tableClass">
					<tr class="firstTr">
						...
						<td class="td2">账号</td>
						<td class="td2">姓名</td>
						<td class="td2">汽车快充数量</td>
						<td class="td2">汽车快充金额</td>
						<td class="td2">汽车快充支付金额</td>
						<td class="td2">汽车快充退款金额</td>
						<td class="td2">汽车慢充数量</td>
						<td class="td2">汽车慢充金额</td>
						<td class="td2">汽车慢充支付金额</td>
						<td class="td2">汽车慢充退款金额</td>
					</tr>
					<tr class="dataTable" v-for="item in cardList" v-show="cardList !== ''">
						<td>{{item.dt}}</td>
						<td>{{item.username}}</td>
						<td>{{item.fullname}}</td>
						<td>{{item.payment_service_11_count}}</td>
						<td>{{item.payment_service_11_amount}}</td>
						<td>{{item.payment_service_11_pay}}</td>
						<td>{{item.payment_service_11_redund}}</td>
						<td>{{item.payment_service_12_count}}</td>
						<td>{{item.payment_service_12_amount}}</td>
						<td>{{item.payment_service_12_pay}}</td>
						<td>{{item.payment_service_12_redund}}</td>
						<!-- <td>
	...
	export default {
		...
		methods: {
			...
			first() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/report/4844243E83074EFB9D7B232364065589?token=" + toKen + "&page=" + this.pagenum +
						"&row=10")
					.then(res => {
						if (res.status == 200) {
			...
			searchData() {
				if (this.isBg == 1) {
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/415826EE04F546FB96A4727209799F35?token=" + toKen + "&page=" + this.pagenum +
							"&row=10&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/FFE50F5238BF4B209CA94F1EBA3158F5?token=" + toKen + "&page=" + this.pagenum +
							"&row=10&from=" + this.value1 + "&to=" + this.value2)
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
