	...
	<div class="oneCard-right">
		...
		<el-dialog title="添加设备" :visible.sync="addDialogVisible" width="30%" @close="addDialogClosed">
			<el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="80px">
                ...
                <el-form-item label="状态" prop="online">
					<el-input v-model="addForm.online" class="addinput"></el-input>
				</el-form-item>
		...
		<el-dialog title="修改设备" :visible.sync="editDialogVisible" width="30%" @close="editDialogClosed">
			<el-form :model="editForm"  ref="editFormRef" label-width="80px">
             ...
             <el-form-item label="状态" prop="value15">
			 	<el-select @change="aalfj" v-model="value15" placeholder="请选择设备状态">
                    <el-option 
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("admin/api/chargers/4/?token=" + toKen + "&page=" + this.pagenum +
					...
					.then(res => {
						...
						if (res.status == 200) {
							this.menuList = res.data.chargers //用户列表数据
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("admin/api/charger/" + id + "?token=" + toKen) //,{"token:toKen"}
					.then(res => {
						console.log(res)
						if (res.status == 200) {
					...
					if (!valid) {
						...
						if(confirm("确认修改吗?")){
							this.$axios.put("admin/api/charger/"+ this.editForm.id,{
								token:toKen,name:this.editForm.name,mac:this.editForm.mac,online:this.value15,longitude:this.editForm.longitude,latitude:this.editForm.latitude,
							})
								.then(res => {
									if (res.status !== 200) {
