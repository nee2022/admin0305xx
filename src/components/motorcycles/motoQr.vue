<template>
	<div class="oneCard-right">
		...
		<!-- 添加设备 -->
		<el-dialog title="添加套餐" :visible.sync="addDialogVisible" width="30%" @close="addDialogClosed">
		...
		<!-- 修改设备 -->
		<el-dialog title="修改套餐" :visible.sync="editDialogVisible" width="30%" @close="editDialogClosed">
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/packages/?token=" + toKen + "&page=" + this.pagenum +
						"&row=14&source=qr&keyword=" + this.input2)
					.then(res => {
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/package", this.addForm)
							.then(res => {
								if (res.status !== 200) {
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("/admin/api/package/" + id + "?token=" + toKen) //,{"token:toKen"}
					.then(res => {
						console.log(res)
						if (res.status == 200) {
					...
					if (!valid) {
						...
						if(confirm("确认修改吗?")){
							this.$axios.put("admin/api/package/"+ this.editForm.id,{
								token:toKen,pay:this.editForm.pay,duration:this.editForm.duration,
							})
								.then(res => {
									if (res.status !== 200) {
