			...
			<div style="height: 680px;">
				<table class="tableClass">
					<tr class="firstTr">
						<td class="td1">日期</td>
						<td class="td2">站点名</td>
						<td class="td2">电单车充电数量</td>
						<td class="td2">电单车充电金额</td>
						<td class="td2">电单车充电支付金额</td>
						<td class="td2">电单车充电退款数</td>
						<td class="td2">电单车充电退款金额</td>
						<td class="td2">图表</td>
					</tr>
					<tr class="dataTable" v-for="item in cardList">
						...
						<td>{{item.refund_service_13_state_1_count}}</td>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				let toKen = this.token.replace(/\"/g, "")
				//console.log(toKen)
				this.$axios.get("admin/api/report/D72839B90D7D40A98EE1EEB4E66DE71F?token=" + toKen + "&page=" + this.pagenum + "&row=10")
					.then(res => {
						console.log(res)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
			...
			daybao(id) {
				...
				if (this.isBg == 1) {
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/1478065A2844423E8ACE2FFC395356CC?token=" + toKen + "&page=" + this.pagenum +
							"&row=10")
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/BEDDD2BEF0364BA483794B9BA60D82F6?token=" + toKen + "&page=" + this.pagenum +
							"&row=10")
						.then(res => {
							if (res.status == 200) {
								this.cardList = res.data.data //用户列表数据
			...
			searchData() {
				if (this.isBg == 1) {
					...
					let toKen = this.token.replace(/\"/g, "")
					// console.log(toKen)
					this.$axios.get("/admin/api/report/D72839B90D7D40A98EE1EEB4E66DE71F?token=" + toKen + "&page=" + this.pagenum +
