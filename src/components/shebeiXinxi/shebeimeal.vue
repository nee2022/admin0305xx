		...
		<div class="tem-right-top">
			...
			<el-dialog :title="name" :visible.sync="stations" width="30%">
				...
				<span slot="footer" class="dialog-footer">
					...
					<el-button type="primary" @click="(stations = false), add()">确 定</el-button>
				</span>
			</el-dialog>
		...
		<div class="right-con">
			<div class="right-con-right">
				...
				<div class="right-con-top">
					<el-select v-model="values" placeholder="请选择套餐级别" class="eees" @change="change">
					...
					<div class="textBox input_right">
						...
						<el-input v-model="msgss" @keyup.enter.native="cha" class="in" placeholder="请输入信息查询"></el-input>
					</div>
					<div class="zhandian">
						<el-button style="width: 100px" type="primary" icon="el-icon-search" class="but but2">查询</el-button>
					</div>
					<div class="zhandian">
						<el-button style="width: 100px" type="primary" icon="el-icon-refresh" class="but but2">重置</el-button>
					</div>
					<div class="zhandian">
						<el-button style="width: 100px" type="primary" icon="el-icon-plus" class="but but2" @click="(stations = true), names()">添加</el-button>
					</div>
				</div>
				<template>
					<el-table :data="parkList" stripe style="width: 100%">
						...
						<el-table-column prop="address" label="操作" width="200" v-if="this.package === 1">
							<template slot-scope="scope">
								<div class="operation">
									<div>
										<el-button type="text" @click="removetapcanByID(scope.row.id)"><img src="../../assets/images/delete.png" /></el-button>
									</div>
									<div>
										<el-button type="text" @click="(stations = true), id(scope.row.id)"><img src="../../assets/images/compile.png" /></el-button>
									</div>
								</div>
							</template>
						</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getParksMes() {
				...
				this.$axios
					.get(
						`admin/api/charger/${this.stationsId}/packages?token=${toKen}&page=${this.newpark}&charger=${this.package}&row=8row=8`
					)
					.then((res) => {
						console.log(res);
