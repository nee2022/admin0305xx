<template>
  <div class="tem-right">
    <div class="tem-right-top">
      <div class="top-left">
        <div class="top-left-word">车牌白名单</div>
      </div>
      ...
      <el-dialog :title="name" :visible.sync="dialogVisible" width="30%">
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="(flag = true), add()"
            >确 定</el-button
          >
        </span>
      </el-dialog>
    ...
    add() {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          if (res.status !== 200) {
            return this.$message.error("添加车牌失败!");
          }
          this.$message.success("添加车牌成功!");
    ...
    getParksMes() {
      ...
      this.$axios
        ...
        .then((res) => {
          ...
          // console.log(res.status)//打印状态码
          if (res.status == 200) {
            this.parkList = res.data.plates; //用户列表数据
