<template>
  <div class="tem-right">
    <div class="tem-right-top">
      <div class="top-left">
        <div class="top-left-word">充电费率</div>
      </div>
...
export default {
  ...
  created() {
    this.stationsId = this.$store.state.rate_group;
  ...
  methods: {
    ...
    getParksMes() {
      ...
      this.$axios
        .get(
          `/admin/api/station/${this.stationsId}/rates?token=${toKen}&page=${this.newpark}&row=8`
        )
        .then((res) => {
          console.log(res);
          // console.log(res.data.users)
          // console.log(res.status)//打印状态码
          if (res.status == 200) {
            this.parkList = res.data.rates; //用户列表数据
