<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">发票(已开票)</span>
			</div>
		...
		<div class="he">
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="dealno" label="发票编号" width="180">
					</el-table-column>
					<el-table-column prop="amount" label="金额">
					</el-table-column>
					<el-table-column prop="tax" label="税费">
					</el-table-column>
					...
					<el-table-column prop="receipt_time" label="开票时间">
					</el-table-column>
				</el-table>
			</template>
		</div>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				console.log(userID)
				this.$axios.get("admin/api/user/" + userID + "/receipts/state/3?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
							this.cardList = res.data.receipts //用户列表数据
