<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">停车记录(历史)</span>
			</div>
		...
		<div>
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="color" label="车牌颜色">
					</el-table-column>
					<el-table-column prop="uid" label="所属用户">
					</el-table-column>
					<el-table-column prop="amount" label="停车费">
					</el-table-column>
					<el-table-column prop="delay_amount" label="违约金">
					</el-table-column>
					<el-table-column prop="paid_type" label="支付类型">
						<template slot-scope="scope">
							<div v-if="scope.row.paid_type == 0" >
								钱包
							</div>
							<div v-if="scope.row.paid_type == 1">
								微信
							</div>
							<div v-if="scope.row.paid_type == 2">
								支付宝
							</div>
						</template>
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				console.log(userID)
				this.$axios.get("admin/api/use/" + userID + "/parked/pdrs?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
