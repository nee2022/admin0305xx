	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				console.log(userID)
				this.$axios.get("/admin/api/user/" + userID + "/plans/1?token=" + toKen + "&page=" + this.pagenum + "&row=12&keyword=" + this.input)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
							this.cardList = res.data.user_plans //用户列表数据
