			...
			<div class="right-con-left">
				<div class="right-con-top">
					<div class="textBox">
						...
						<el-input placeholder="请输入关键字查找" class="textWord" clearable v-model="input1" clearable @keyup.enter.native="getRoadMes"></el-input>
					</div>
					<div>
	...
	export default {
		...
		methods: {
			...
			getRoadMes() {
				...
				// console.log(toKen)
				this.$axios.get("admin/api/lot_rate/groups?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input1)
					.then(res => {
			...
			getParksMes(id, name) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("/admin/api/lot_rate/group/" + id + "/lot_rates?token=" + toKen + "&page=" + this.newpark +
						"&row=14")
					.then(res => {
			...
			getParksMe() {
				...
				this.$axios.get("/admin/api/lot_rate/group/" + this.getFristID + "/lot_rates?token=" + toKen + "&page=" + this.newpark + "&row=14")
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/lot_rate?token="+this.addForm.token+"&name=" + this.addForm.name + "&price="+ this.addForm.price + "&start_time="+ this.addForm.start_time + "&end_time=" + this.addForm.end_time + "&group=" + this.getFristID)
							.then(res => {
								if (res.status !== 200) {
			...
			chargersUser1() {
				...
				this.$refs.addFormRef1.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/lot_rate/group", this.addForm1)
							.then(res => {
								if (res.status !== 200) {
									return this.$message.error('添加数据失败!')
								}
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("/admin/api/lot_rate/" + id + "?token=" + toKen)
					.then(res => {
			...
			async removeUserByID1(id) {
				...
				this.$axios.delete("/admin/api/lot_rate/group/" + id + "?token=" + toKen)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
							this.$message.success('删除数据成功')
			...
			showEditDialog(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("/admin/api/lot_rate/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
			...
			showEditDialog1(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("/admin/api/lot_rate/group/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
			...
			editChargerInfo1() {
				...
				this.$refs.editFormRef1.validate(valid => {
					if (!valid) {
						...
						if (confirm("确认修改吗?")) {
							this.$axios.put("/admin/api/lot_rate/group/" + this.editForm1.id, {
									token: toKen,
