					...
					<el-table :data="tableData" stripe style="width:100%">
						...
						<el-table-column prop="name" label="套餐">
							<template slot-scope="scope">
								<div @click="getParksMes(scope.row.id,scope.row.name)">{{scope.row.name}}</div>
							</template>
						</el-table-column>
			...
			<div class="right-con-right">
				...
				<div>
					<template>
						...
						<el-table-column prop="trigger_evt" label="触发事件">
							<template slot-scope="scope">
								<div v-if="scope.row.trigger_evt == 0">
									无
								</div>
								<div v-else-if="scope.row.trigger_evt == 1">
									注册
								</div>
								<div v-else-if="scope.row.trigger_evt == 2">
									充值
								</div>
								<div v-else-if="scope.row.trigger_evt == 3">
									业务消费
								</div>
							</template>
						</el-table-column>
						...
						<el-table-column prop="avail_from" label="活动开始">
						</el-table-column>
						<el-table-column prop="avail_to" label="活动结束">
						</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getRoadMes() {
				...
				// console.log(toKen)
				this.$axios.get("admin/api/plan/groups?token=" + toKen + "&page=" + this.pagenum + "&row=14&keyword=" + this.input1)
					.then(res => {
						console.log(res.data)
						// console.log(res.data.users)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.tableData = res.data.plan_groups //用户列表数据
			...
			getParksMes(id, name) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("admin/api/plan/group/" + id + "/plans?token=" + toKen + "&page=" + this.newpark +
						"&row=14")
					.then(res => {
						console.log(res.data.plans)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.parkList = res.data.plans //用户列表数据
			...
			getParksMe() {
				...
				this.$axios.get("admin/api/plan/group/" + this.getFristID +"/plans?token=" + toKen + "&page=" + this.newpark + "&row=14")
					.then(res => {
						console.log(res.data)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/plan?token="+this.addForm.token+"&name=" + this.addForm.name + "&amount="+ this.addForm.amount + "&group=" + this.getFristID + "&type=2")
							.then(res => {
								if (res.status !== 200) {
			...
			chargersUser1() {
				...
				this.$refs.addFormRef1.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/plan/group", this.addForm1)
							.then(res => {
								if (res.status !== 200) {
									return this.$message.error('添加数据失败!')
								}
			...
			async removeUserByID1(id) {
				...
				this.$axios.delete("/admin/api/plan/group" + id + "?token=" + toKen)
					.then(res => {
						console.log(res)
						if (res.status == 200) {
							this.$message.success('删除数据成功')
			...
			showEditDialog(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("/admin/api/plan/" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
							this.editForm = res.data.plan
			...
			showEditDialog1(id) {
				...
				let toKen = this.token.replace(/\"/g, "")
				// console.log(toKen)
				this.$axios.get("/admin/api/plan/group" + id + "?token=" + toKen) //根据id点击修改的id查询设备信息
					.then(res => {
						//console.log(res.status)
						if (res.status == 200) {
							this.editForm1 = res.data.plan_group
						}
					})
			},
			...
			editChargerInfo() {
				...
				this.$refs.editFormRef.validate(valid => {
					if (!valid) {
						...
						if (confirm("确认修改吗?")) {
							this.$axios.put("/admin/api/plan/" + this.editForm.id, {
									token: toKen,
			...
			editChargerInfo1() {
				...
				this.$refs.editFormRef1.validate(valid => {
					if (!valid) {
						...
						if (confirm("确认修改吗?")) {
							this.$axios.put("/admin/api/plan/group/" + this.editForm1.id, {
									token: toKen,
									name: this.editForm1.name,
									memo:this.editForm1.memo
								})
								.then(res => {
									if (res.status !== 200) {
