<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">包月套餐</span>
			</div>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("admin/api/plans/1?token=" + toKen + "&page=" + this.pagenum +
					...
					.then(res => {
						...
						if (res.status == 200) {
							this.menuList = res.data.plans //用户列表数据
			...
			chargersUser() {
				...
				this.$refs.addFormRef.validate(valid => {
					if (!valid) {
						...
						this.$axios.post("/admin/api/plan", this.addForm)
							.then(res => {
								if (res.status !== 200) {
			...
			async removeUserByID(id) {
				...
				this.$axios.delete("/admin/api/plan/" + id + "?token=" + toKen) //,{"token:toKen"}
					.then(res => {
						console.log(res)
						if (res.status == 200) {
					...
					if (!valid) {
						...
						if(confirm("确认修改吗?")){
							this.$axios.put("admin/api/plan/"+ this.editForm.id,{
								token:toKen,name:this.editForm.name,pay:this.editForm.pay,amount:this.editForm.amount,
							})
								.then(res => {
									if (res.status !== 200) {
