<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">管理员日志</span>
			</div>
		...
		<div class="UserAssets-right-text">
			...
			<template>
				<div class="block">
					<el-date-picker v-model="value1" type="date" placeholder="选择日期" @change="chooseDate" value-format="yyyyMMdd">
					</el-date-picker>
				</div>
			</template>
			<div>
		...
		<div>
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="operator" label="账号">
					</el-table-column>
      ...
      this.getUserMes()
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/operator/logs?token=" + toKen + "&page=" + this
						.pagenum + "&row=14&dt="+ this.value1)
					.then(res => {
						console.log(res.data)
						console.log(res.data.logs)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.cardList = res.data.logs //用户列表数据
