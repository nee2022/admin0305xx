<template>
	<div class="UserAssets-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">事件记录</span>
			</div>
		...
		<div>
			<template>
				<el-table :data="tableData" stripe style="width: 100%">
					<el-table-column prop="id" label="节点ID" width="130">
					</el-table-column>
					<el-table-column prop="object" label="对象ID" width="180">
					</el-table-column>
					<el-table-column prop="time" label="时间">
					</el-table-column>
					<el-table-column prop="code" label="设备事件">
					</el-table-column>
					<el-table-column prop="class" label="事件类型">
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/monitor/api/class/device/events?token=" + toKen + "&page=" + this.pagenum + "&row=14")
					.then(res => {
						console.log(res.data.events)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.tableData = res.data.events //用户列表数据
