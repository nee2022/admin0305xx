<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">车辆认证(审核中)</span>
			</div>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/plate/certificates/state/0?token=" + toKen + "&page=" + this.pagenum +
						"&row=14")
					.then(res => {
			...
			shenheFalse(id){
				...
				// console.log(toKen)
				if(confirm("确认审核失败吗？")){
					this.$axios.put("admin/api/plate/certificate/"+ id,{token:toKen,audit:false,memo:"审核失败"})
						.then(res => {
							if(res.data.error !== 0){
			...
			shenheTrue(id){
				...
				// console.log(toKen)
				if(confirm("确认审核成功吗？")){
					this.$axios.put("admin/api/plate/certificate/"+ id,{token:toKen,audit:true,memo:"审核成功"})
						.then(res => {
							if(res.data.error !== 0){
								this.$message.error("数据更新失败")
							}else{
			...
			shenheFalse(id,plate) {
				//token去掉引号
				let toKen = this.token.replace(/\"/g, "")
				if (confirm("确认审核失败吗？")) {
					this.$axios.put("admin/api/plate/certificate/" + id, {
							token: toKen,
							audit: false,
							plate:plate,
							memo: "审核失败"
