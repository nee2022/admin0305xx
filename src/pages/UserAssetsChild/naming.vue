<template>
	<div class="UserAssets-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">实名认证</span>
			</div>
		...
		<div>
			<template>
				<el-table :data="tableData" stripe style="width: 100%">
					...
					<el-table-column prop="id" label="认证ID" width="180">
					</el-table-column>
					...
					<el-table-column prop="address" label="操作" width="200">
						<template slot-scope="scope">
							<div class="operation">
								<div>
									<button type="text"><img src="../../assets/images/delete.png" @click="shenheFalse(scope.row.id)"/></button>
								</div>
								<div>
									<button type="text"><img src="../../assets/images/right.png" style="width: 15;height: 11px;" @click="shenheTrue(scope.row.id)" /></button>
								</div>
								<div>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/user/certificates/state/0?token=" + toKen + "&page=" + this.pagenum + "&row=14")
					.then(res => {
						console.log(res.data.users)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.tableData = res.data.user_certificates //用户列表数据
			...
			shenheFalse(id){
				...
				// console.log(toKen)
				if(confirm("确认审核失败吗？")){
					this.$axios.put("admin/api/user/certificate/"+ id,{token:toKen,audit:false,memo:"审核失败"})
						.then(res => {
							if(res.data.error !== 0){
			...
			shenheTrue(id){
				...
				// console.log(toKen)
				if(confirm("确认审核成功吗？")){
					this.$axios.put("admin/api/user/certificate/"+ id,{token:toKen,audit:true,memo:"审核成功"})
						.then(res => {
							if(res.data.error !== 0){
								this.$message.error("数据更新失败")
							}else{
