<template>
	<div class="oneCard-right">
		<div class="UserAssets-right-top">
			<div class="user-left">
				<span class="user-word">无感支付</span>
			</div>
				...
				<el-table :data="cardList" stripe style="width: 100%">
					...
					<el-table-column prop="gateway" label="支付网关">
						<template slot-scope="scope">
								...
								<div v-else-if="scope.row.gateway == 'wechat'" class="stateColor-green">
									微信支付
								</div>
								<div v-else-if="scope.row.gateway == 'alipay'" class="stateColor-green">
									支付宝
								</div>
								<div v-else-if="scope.row.gateway == 'ckej'" class="stateColor-green">
									城客e家
								</div>
								<div v-else class="stateColor-green">
									其他
								</div>
							</template>
					</el-table-column>
					...
					<el-table-column prop="amount" label="无感金额">
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				// console.log(toKen)
				this.$axios.get("/admin/api/trustpay/pdrs?token=" + toKen + "&page=" + this.pagenum + "&row=14")
					.then(res => {
						console.log(res.data)
						console.log(res.data.trustpays)
						// console.log(res.status)//打印状态码
						if (res.status == 200) {
							this.cardList = res.data.trustpays //用户列表数据
