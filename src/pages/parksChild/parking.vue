		...
		<div>
			<template>
				<el-table :data="cardList" stripe style="width: 100%">
					<el-table-column prop="id" label="停车编号" width="130">
					</el-table-column>
					<el-table-column prop="plate" label="车牌号码" width="180">
					</el-table-column>
					<el-table-column prop="confirm_operator_no" label="收费取证员编号">
					</el-table-column>
					<el-table-column prop="station_name" label="场地名称">
					</el-table-column>
					<el-table-column prop="enter_time" label="驶入时间">
					</el-table-column>
					<el-table-column prop="enter_type" label="驶入时间来源">
                        <template slot-scope="scope">
							<div v-if="scope.row.enter_type == '0' ">
								系统
							</div>
                            <div v-else-if="scope.row.enter_type == '1' ">
								管理员
							</div>
                            <div v-else-if="scope.row.enter_type == '2' ">
								运营商
							</div>
                            <div v-else-if="scope.row.enter_type == '3' ">
								用户
							</div>
                            <div v-else-if="scope.row.enter_type == '4' ">
								收费员
							</div>
                            <div v-else-if="scope.row.enter_type == '5' ">
								抓拍机
							</div>
                            <div v-else-if="scope.row.enter_type == '6' ">
								地磁
							</div>
                            <div v-else-if="scope.row.enter_type == '7' ">
								高位视频
							</div>
                            <div v-else-if="scope.row.enter_type == '8' ">
								低位视频
							</div>
                            <div v-else-if="scope.row.enter_type == '9' ">
								巡检车
							</div>
                            <!-- v-if的值为false时，v-else模块才会显示出来。 -->
							<div v-else>
								{{scope.row.enter_type}}
							</div>
						</template>
					</el-table-column>
					<el-table-column prop="duration" label="停车时间">
					</el-table-column>
	...
	export default {
		...
		methods: {
			...
			getUserMes() {
				...
				//console.log(toKen)
				this.$axios.get("admin/api/pdrs/service/21/state/1/?token=" + toKen + "&page=" + this.pagenum + "&row=14")
					.then(res => {
                        ...
                        //     this.enter_typesava.push(this.cardList[i].enter_type)
                        // }
                        // console.log(this.enter_typesava)
					})
			},
